...
if __name__ == '__main__':
    ...
    for l in letters:
        ...
        directory = "data1/train/" + l
        aug_directory = "data1/train/" + l
        ...
        for i, filename in enumerate(random_items):
            if filename.endswith(".png"):
                ...
                image_pil = Image.fromarray((noisy_im * 255).astype(np.uint8))

                # Save the image as a PNG file
                image_pil.save(aug_directory + f"/nosiy_aug_{i}.png")
        ...
        for i, filename in enumerate(random_items):
            if filename.endswith(".png"):
                ...
                # Save the image as a PNG file
                image_pil.save(aug_directory + f"/lines_aug_{i}.png")
        ...
        for i, filename in enumerate(random_items):
            if filename.endswith(".png"):
                ...
                # Plot the image
                image = plt.imread(file_path)
                noisy_im = rotate_image(image)

                image_pil = Image.fromarray((noisy_im * 255).astype(np.uint8))

                # Save the image as a PNG file
                image_pil.save(aug_directory + f"/rotate_aug_{i}.png")
        ...
        for i, filename in enumerate(random_items):
            if filename.endswith(".png"):
                ...
                # Plot the image
                image = plt.imread(file_path)
                zoom_in_im = zoom_in(image)

                image_pil = Image.fromarray((zoom_in_im * 255).astype(np.uint8))

                # Save the image as a PNG file
                image_pil.save(aug_directory + f"/zoomin_aug_{i}.png")
        ...
        for i, filename in enumerate(random_items):
            if filename.endswith(".png"):
                ...
                # Plot the image
                image = plt.imread(file_path)
                zoom_in_im = zoom_out(image)

                image_pil = Image.fromarray((zoom_in_im * 255).astype(np.uint8))

                # Save the image as a PNG file
                image_pil.save(aug_directory + f"/zoomout_aug_{i}.png")

        # mix1 - noisy & zoom in & rotate aug
        # Randomly select 100 items from the list
        random_items = random.sample(items, k=100)
        for i, filename in enumerate(random_items):
            if filename.endswith(".png"):
                ...
                rotated_im = rotate_image(zoom_in_im)

                image_pil = Image.fromarray((rotated_im * 255).astype(np.uint8))

                # Save the image as a PNG file
                image_pil.save(aug_directory + f"/mix1_aug_{i}.png")


        # mix2 - rotate & lines & zoom out aug
        # Randomly select 100 items from the list
        random_items = random.sample(items, k=100)
        for i, filename in enumerate(random_items):
            if filename.endswith(".png"):
                ...
                zoom_out_im = zoom_out(lines_im)

                image_pil = Image.fromarray((zoom_out_im * 255).astype(np.uint8))

                # Save the image as a PNG file
                image_pil.save(aug_directory + f"/mix2_aug_{i}.png")


        # mix3 - zoom out & noisy & zoom in & lines & rotate aug
        # Randomly select 100 items from the list
        random_items = random.sample(items, k=100)
        for i, filename in enumerate(random_items):
            if filename.endswith(".png"):
                ...
                noisy_im = noisy_aug(zoom_out_im)
                zoom_in_im = zoom_in(noisy_im)
                lines_im = lines_aug(zoom_in_im)
                rotated_im = rotate_image(lines_im)

                image_pil = Image.fromarray((rotated_im * 255).astype(np.uint8))

                # Save the image as a PNG file
                image_pil.save(aug_directory + f"/mix3_aug_{i}.png")
